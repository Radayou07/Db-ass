...
export default function Home() {
  ...
  if (loading) return (
    <div className="h-full flex items-center justify-center text-sky-500 animate-pulse">
      <FiActivity size={40} />
    </div>
  )
  ...
  return (
    <div className="p-6 flex flex-col gap-8 pb-20">
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="w-full lg:w-[260px] shrink-0 flex flex-col gap-4">
              ...
              {categories.slice(0, 8).map((cat) => (
                ...
                >
                  <span className="flex items-center gap-3">
                    {/* Placeholder icon logic based on index or name could go here */}
          ...
          <div className="bg-slate-100 dark:bg-slate-800/50 rounded-2xl p-6 text-center border border-black/5 dark:border-white/5 border-dashed">
            ...
            <Link to="/products" className="inline-block px-6 py-2.5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 text-[10px] font-black uppercase tracking-widest rounded-xl hover:scale-105 transition-transform">
              Shop Now
            </Link>
          </div>
        </div>
          ...
          <div className="relative w-full h-[400px] lg:h-[480px] bg-slate-100 dark:bg-slate-800 rounded-3xl overflow-hidden shadow-sm flex items-center">
            ...
            <div className="relative z-10 px-10 md:px-16 flex flex-col items-start gap-4">
               ...
               <div className="flex items-center gap-6 mt-auto bg-white/80 dark:bg-slate-900/80 backdrop-blur-md p-6 rounded-3xl border border-white/50 shadow-xl">
                 ...
                 <Link to="/products" className="ml-4 px-8 py-4 bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-black text-sm uppercase tracking-widest hover:-translate-y-1 transition-transform shadow-lg rounded-xl">
                   Shop Now!
                 </Link>
               </div>
            </div>
            
            {/* Model Image Placeholder (Right Side) */}
            <div className="absolute right-0 bottom-0 h-[90%] w-1/2 hidden md:block">
              {/* This represents where the model image would go */}
      ...
      {/* ─── NEW ARRIVALS PREVIEW (Optional, just keeping it from old home) ─── */}
      {featuredProducts.length > 0 && (
        <div className="mt-8 flex flex-col gap-6">
          <div className="flex items-center justify-between border-b border-black/5 dark:border-white/5 pb-4">
            ...
            <Link to="/products" className="text-xs font-bold text-slate-400 hover:text-sky-500 uppercase tracking-widest transition-colors">View All &rarr;</Link>
          </div>
          
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {featuredProducts.map(product => (
              <Link to={`/products`} key={product.id} className="group flex flex-col gap-3">

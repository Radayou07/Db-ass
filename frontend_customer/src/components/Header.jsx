...
import { FiSearch, FiPhoneCall, FiUser, FiHeart, FiShoppingCart, FiChevronDown, FiMenu } from 'react-icons/fi'
...
export default function Header({ isDark, setIsDark }) {
  ...
  const handleSearch = (e) => {
    e.preventDefault()
    // Ideally this would navigate to a search page or products page with params
    ...
    <header className="flex flex-col w-full bg-white dark:bg-slate-900 border-b border-black/5 dark:border-white/5">
      ...
      <div className="w-full py-5 px-6 flex items-center justify-between gap-8 max-w-7xl mx-auto">
        ...
        <div className="flex items-center gap-6 shrink-0">
          ...
          <div className="flex items-center gap-4 text-slate-700 dark:text-slate-200">
            <Link to="/profile" className="flex flex-col items-center gap-1 hover:text-sky-500 transition-colors">
              <FiUser size={24} />
            ...
            <Link to="/cart" className="relative flex flex-col items-center gap-1 hover:text-sky-500 transition-colors cursor-pointer">
      ...
      <div className="w-full border-t border-black/5 dark:border-white/5 bg-white dark:bg-slate-900">
        <div className="max-w-7xl mx-auto px-6 flex items-center gap-8">
          <Link to="/" className={`py-4 text-xs font-black uppercase tracking-widest border-b-2 transition-colors ${location.pathname === '/' ? 'border-sky-500 text-sky-500' : 'border-transparent text-slate-600 dark:text-slate-300 hover:text-sky-500'}`}>Home</Link>
          <Link to="/products" className={`py-4 text-xs font-black uppercase tracking-widest border-b-2 transition-colors ${location.pathname === '/products' ? 'border-sky-500 text-sky-500' : 'border-transparent text-slate-600 dark:text-slate-300 hover:text-sky-500'}`}>Products</Link>
          <Link to="/orders" className={`py-4 text-xs font-black uppercase tracking-widest border-b-2 transition-colors ${location.pathname === '/orders' ? 'border-sky-500 text-sky-500' : 'border-transparent text-slate-600 dark:text-slate-300 hover:text-sky-500'}`}>Orders</Link>
